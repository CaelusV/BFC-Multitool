...
use iced::{
	Alignment::Center, Element, Font, Task, font, task, widget::{button, column, progress_bar, row, text}
};
use shared::Progress;
use strum_macros::Display;

use crate::{MARGIN, Message};
...
pub enum ToolError {
    ...
    RosterFileError(RosterFileError),
}
...
enum WorkState {
    #[default]
    Idle,
    Working { progress: f32, _task: task::Handle},
    Finished,
    Errored,
}
...
impl Tools {
    pub fn start(&mut self, tool: Tool, source: PathBuf, destination: PathBuf) -> Task<WorkUpdate> {
        ...
        match tool_state.state {
            WorkState::Idle | WorkState::Finished | WorkState::Errored => {
                let (task, handle) = Task::sip(lineupper::create::create_team_and_portraits(source, destination), WorkUpdate::Working, WorkUpdate::Finished).abortable();
    //             let (task, handle) = match tool {
                ...
                tool_state.state = WorkState::Working { progress: 0.0, _task: handle.abort_on_drop(), };
                task
            }
    ...
    pub fn update(&mut self, update: WorkUpdate) {}
	...
	fn tool_section(&self, tool: Tool) -> Element<'_, Message> {
		...
		let progress = match work_state {
		    WorkState::Idle | WorkState::Errored => 0.0,
			WorkState::Working { progress, _task } => progress,
			WorkState::Finished => 100.0,
		};

		let tool_status_text = if progress == 100.0 {
            "Done!"
        } else if source_path.is_some() && destination_path.is_some() {
            "Ready!"
        } else {
            "Missing path(s)."
        };
		...
		].align_y(Center).spacing(MARGIN * 2.0);
