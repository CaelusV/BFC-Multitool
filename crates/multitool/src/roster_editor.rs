...
impl RosterEditor {
	pub fn editor(&mut self, ui: &mut Ui) {
		...
		egui::Frame::window(ui.style())
			.inner_margin(Margin::symmetric(6.0, 5.0)) // x breaks striped if not same as outer_margin, or if spacing.x too high.
			...
			.show(ui, |ui| {
				ScrollArea::vertical().show(ui, |ui| {
					TableBuilder::new(ui)
						...
						.column(Column::remainder().at_least(50.0)) // Active.
						.striped(true)
