...
pub async fn run_tournaments(source: PathBuf, destination: PathBuf) -> impl Straw<(), Progress, Error> {
    sipper(async move |mut progress| {
    	...
    	for cup in cup_paths {
            let _ = progress.send(Progress { percent: percent_done }).await;
        ...
        let _ = progress.send(Progress { percent: percent_done }).await; // Need to update final cup processed.
...
async fn get_cup_paths(source: &PathBuf) -> Result<Vec<PathBuf>> {
